...
describe('MERGE', () => {
  test('simple MERGE', () => {
    const query =
      /* cypher */`MERGE (u:USER { username: "test"})-[r:CREATED]->(p:PRODUCT { id: 1 }) RETURN p;`;
...
RETURN p;`);
  ...
  test('MERGE with ON MATCH', () => {
    const query =
      /* cypher */`MERGE (u:USER { username: "test"})-[r:CREATED]->(p:PRODUCT { id: 1 }) ON MATCH SET p += $param_1, u += $param_2, p.key_1 = "value" RETURN p;`;
